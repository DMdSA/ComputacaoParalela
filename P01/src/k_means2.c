...
int update_samples(const int samples, const int klusters) {
    ...
    int lastK = 0, minK = 0, changes = 0, point = 0;
    float* distances = malloc(sizeof(float) * samples * klusters);
    ...
    for(int k = 0; k < klusters; k++){
        for(int i = 0; i < samples; i++){
            sp = &(RANDOM_SAMPLE[i]);
            dist = distances[(samples*k)+i];
            printf("%f -- %d\n",sp->dist,samples * k + i);
            if(dist < sp->dist){
                if(sp->k != -1)((CLUSTERS[sp->k]).dimension)--;
                (CLUSTERS[k].dimension)++;
                sp->dist = dist;
                sp->k = k;
