...
/**
 * @brief @brief gets the distance between two points, without square rooting.
...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    if (rank == 0) {
        // update all centroids (root)
        updateCentroids(changesRecv,n_clusters);
    }

    // share updated clusters among all processes
    // obs: the correct dimensions (for each process) are stored in "previousDimensions"
    MPI_Bcast(CLUSTERS, n_clusters, mpi_cluster, 0, MPI_COMM_WORLD);

    // recover dimensions (each process)
    for(int i = 0; i < n_clusters; i++) {
    ...
    while(changes_flag) {
        ...
        changes_flag = update_samples(SAMPLE_SUBSET, changesSend, number_points_per_proccess, n_clusters);
        
        // has any change been made by any of the clusters?
        MPI_Allreduce(MPI_IN_PLACE, &changes_flag, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

        // if so, root will do some work...
        if (changes_flag != 0){
            
            // save real dimensions for each process
            for(int i = 0; i < n_clusters; i++)
                previousDimensions[i] = CLUSTERS[i].dimension = (int) changesSend[i*3];

            // reduce auxiliar changes array for root process
            MPI_Reduce(changesSend, changesRecv, arraysize, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);

            if (rank == 0)
                updateCentroids(changesRecv,n_clusters);
            
            // share root's work with everyone
            MPI_Bcast(CLUSTERS, n_clusters, mpi_cluster, 0, MPI_COMM_WORLD);
